...
public class GsonTests {
    ...
    public void oneUserTest() throws Exception {
        ...
        final IUserParser pUserParserGson = new UserParserGson(inputStreamToString(response));
        final IUser pUser = pUserParserGson.parse();
    ...
    @Test
    public void userArrayTest() throws Exception {
        final InputStream mockInputStream = MockHttpClient.mockRequest("src\\test\\resources\\user_list.json");
        when(mHttpClient.request(Matchers.anyString())).thenReturn(mockInputStream);
        final InputStream response = mHttpClient.request("http://take.user.com/api");

        final IUsersListParserGson pUserParserGson = new UsersListParserGson(response);
        final IUsersListGson pUserList = pUserParserGson.parse();
